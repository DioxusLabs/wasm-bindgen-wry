...
// `wasm_bindgen` names consumers reach through `use wasm_bindgen_futures::*`
// are the same crates as their own patched dependencies; rustc accepts a
// glob/extern-prelude overlap only when both names resolve to the same crate.
// The `::` prefix resolves from the extern prelude rather than the
// glob-imported names.
pub use ::js_sys;
